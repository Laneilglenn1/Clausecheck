...
const FREE_LIMIT = 3;
const STORAGE_KEY = "ss_analyses";
const PAID_KEY = "ss_paid";

function getUsageCount() {
  try { return parseInt(localStorage.getItem(STORAGE_KEY) || "0"); } catch { return 0; }
}
function incrementUsage() {
  try { localStorage.setItem(STORAGE_KEY, String(getUsageCount() + 1)); } catch {}
}
function isPaid() {
  try { return localStorage.getItem(PAID_KEY) === "true"; } catch { return false; }
}
function setPaid() {
  try { localStorage.setItem(PAID_KEY, "true"); } catch {}
}
...
// Paywall screen
function Paywall({ usageCount }) {
  const [loading, setLoading] = useState(false);
  return (
    <div style={{ textAlign: "center", padding: "40px 20px", animation: "fadeUp 0.3s ease" }}>
      <div style={{ fontSize: 48, marginBottom: 16 }}>🛡️</div>
      <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: "-0.5px", marginBottom: 8 }}>You've used your {FREE_LIMIT} free analyses</div>
      <div style={{ fontSize: 15, color: C.subtle, marginBottom: 32, lineHeight: 1.6, maxWidth: 380, margin: "0 auto 32px" }}>
        Upgrade to SignShield Pro for unlimited contract analysis, red flag detection, negotiation emails, and AI attorney chat.
      </div>

      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: "28px 24px", maxWidth: 360, margin: "0 auto 24px", textAlign: "left" }}>
        <div style={{ fontSize: 12, color: C.accent, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>SignShield Pro</div>
        <div style={{ fontSize: 42, fontWeight: 800, letterSpacing: "-2px", marginBottom: 4 }}>$29<span style={{ fontSize: 16, fontWeight: 500, color: C.muted, letterSpacing: 0 }}>/month</span></div>
        <div style={{ fontSize: 13, color: C.muted, marginBottom: 20 }}>Cancel anytime</div>
        {["Unlimited contract analyses", "Red flag detection & severity scoring", "Missing clause alerts", "AI negotiation email generator", "AI attorney chat", "Downloadable analysis reports"].map((f, i) => (
          <div key={i} style={{ display: "flex", gap: 8, fontSize: 13, color: C.subtle, paddingBottom: 7, lineHeight: 1.5 }}>
      ...
      <button
        ...
        disabled={loading}
        style={{ width: "100%", maxWidth: 360, background: `linear-gradient(135deg,${C.accent},${C.purple})`, color: "#fff", border: "none", borderRadius: 11, padding: "16px 0", fontSize: 16, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1 }}>
        {loading ? "Loading..." : "Subscribe for $29/month →"}
      </button>
      <div style={{ fontSize: 12, color: C.muted, marginTop: 12 }}>Secure payment via Stripe · Cancel anytime</div>
    </div>
  );
}
...
function downloadReport(result) {
  const lines = ["SIGNSHIELD CONTRACT ANALYSIS REPORT", "=====================================", "", "Contract Type : " + result.contractType, "Risk Level    : " + result.overallRisk + " (" + result.riskScore + "/10)", "Parties       : " + (result.parties || []).join(" / "), "", "--- VERDICT ---", result.verdict, "", "--- SUMMARY ---", result.summary, "", "--- RED FLAGS ---"];
  (result.flags || []).forEach((f, i) => { lines.push("", (i + 1) + ". [" + f.severity + "] " + f.title, "   Issue: " + f.explanation, "   Fix:   " + f.recommendation); });
...
export default function App() {
  ...
  const [paid, setPaidState] = useState(isPaid());
  ...
  async function sendChat() {
    ...
    const sys = "You are a contract attorney. Contract: Type: " + result?.contractType + ". Risk: " + result?.riskScore + "/10. Verdict: " + result?.verdict + ". Flags: " + (result?.flags || []).map(f => f.title).join(", ") + ". Answer concisely.";
    try {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Inter',system-ui,sans-serif", display: "flex", flexDirection: "column", alignItems: "center", padding: "0 16px 80px" }}>
          ...
          {!paid && (
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ fontSize: 12, color: C.muted }}>
                <span style={{ color: remaining === 0 ? C.danger : C.warn, fontWeight: 700 }}>{remaining}</span> free {remaining === 1 ? "analysis" : "analyses"} left
              </div>
              <button onClick={startCheckout} style={{ background: `linear-gradient(135deg,${C.accent},${C.purple})`, color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                Go Pro $29/mo
              </button>
            </div>
          )}
          {paid && (
            <div style={{ fontSize: 12, color: C.ok, fontWeight: 600 }}>✓ Pro — Unlimited</div>
          )}
        </div>
      </div>
      ...
      <div style={{ width: "100%", maxWidth: 800 }}>
        ...
        {/* Paywall */}
        {showPaywall && <Paywall usageCount={usageCount} />}
        ...
        {!result && !loading && !showPaywall && (
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            {!paid && (
              <div style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", marginTop: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 13, color: C.subtle }}>
                  {remaining > 0 ? `${remaining} free ${remaining === 1 ? "analysis" : "analyses"} remaining` : "No free analyses left — upgrade to continue"}
                </span>
                {remaining === 0 && (
                  <button onClick={startCheckout} style={{ background: C.accent, color: "#fff", border: "none", borderRadius: 7, padding: "6px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Upgrade</button>
                )}
              </div>
            )}
