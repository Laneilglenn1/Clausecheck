...
const C = {
  ...
  border: "#1C2640", accent: "#3B6EFF", danger: "#F04F58",
  warn: "#F5A623", ok: "#1FC87A", text: "#EEF2FF",
  muted: "#4A5578", subtle: "#7B88B0", purple: "#8B5CF6",
};

// Calls YOUR server (/api/analyze), which holds the secret key.
...
function downloadReport(result) {
  ...
  lines.push("");
  lines.push("Generated by ClauseCheck AI - " + new Date().toLocaleDateString());
...
export default function App() {
  ...
  async function analyze(contractText) {
    ...
    const prompt = "You are a senior contract attorney. Analyze the contract at the end of this message.\n\n" +
      ...
      "Required fields:\n" +
      "contractType (string), parties (array of 2 strings), summary (string), overallRisk (HIGH/MEDIUM/LOW), riskScore (number 1-10), " +
      "flags (array of {severity, title, explanation, recommendation}), " +
      "missingClauses (array of {name, why}), positiveClauses (array of strings), " +
  ...
  async function generateNeg() {
    ...
    try {
      const t = await callClaude([{ role: "user", content: prompt }], null, 1024);
      setNegotiation(t);
    } catch (e) {
      setNegotiation("Failed to generate. Try again.");
    } finally {
      setLoadingNeg(false);
    }
  }
  ...
  async function sendChat() {
    ...
    setChatHistory(hist);
    setChatLoading(true);
    const sys = "You are a contract attorney. This contract was analyzed: Type: " + result?.contractType +
      ". Risk: " + result?.riskScore + "/10. Verdict: " + result?.verdict +
      ". Key flags: " + (result?.flags || []).map(f => f.title).join(", ") +
    ...
    try {
      ...
      setChatHistory([...hist, { role: "assistant", content: "Error: " + e.message }]);
    } finally {
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Inter',system-ui,sans-serif", display: "flex", flexDirection: "column", alignItems: "center", padding: "0 16px 80px" }}>
      <style>{`
        ...
        *{box-sizing:border-box} body{margin:0} textarea,input{outline:none;font-family:inherit}
        textarea:focus,input:focus{border-color:#3B6EFF!important}
      ...
      <div style={{ width: "100%", maxWidth: 800, paddingTop: 38, paddingBottom: 26, borderBottom: `1px solid ${C.border}`, marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 5 }}>
          ...
          <span style={{ fontSize: 19, fontWeight: 800, letterSpacing: "-0.5px" }}>ClauseCheck</span>
          <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 6px", borderRadius: 4, background: C.accent + "22", color: C.accent }}>AI</span>
        </div>
        <div style={{ fontSize: 13, color: C.muted }}>Contract analysis, red flags, negotiation scripts — in 60 seconds</div>
      </div>
      ...
      <div style={{ width: "100%", maxWidth: 800 }}>
        ...
        {result && (
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            {tab === "Negotiate" && (
              <div>
                <div style={{ fontSize: 13, color: C.subtle, marginBottom: 14, lineHeight: 1.6 }}>Generate a professional negotiation email based on this contract's issues — ready to copy and send.</div>
                {!negotiation && (
                  <button onClick={generateNeg} disabled={loadingNeg} style={{ width: "100%", background: loadingNeg ? C.border : `linear-gradient(135deg,${C.accent},${C.purple})`, color: loadingNeg ? C.muted : "#fff", border: "none", borderRadius: 11, padding: "14px 0", fontSize: 14, fontWeight: 700, cursor: loadingNeg ? "not-allowed" : "pointer" }}>
                    {loadingNeg ? "Drafting email..." : "✍️ Generate Negotiation Email"}
                  </button>
                )}
                {negotiation && (
                  <>
